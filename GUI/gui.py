...
def addXBeeInput(input, data): # Takes input (string) and appends it to data (list of lists)
    ...
    return data
def storeSimPressureData(): # Reads simulation pressure data
...
def storeBackupData(): # Reads backup data
    backupData = []
    ...
    return backupData  
def getBackupData(name):  # IS THIS RIGHT? Creates the data from the simulation file
    if(name == "altitude"):
        tempYData = []
...
#Global variables
data = [] # Stores all telemetry data (list of lists) -- maybe make it a dictionary of lists?
backupData = storeBackupData() # Stores the backup data (read from a file)
simulatedPressureData = storeSimPressureData() # Stores the simulated pressure data (read from a file)
TEAM_ID = '1071'
MISSION_TIME = 0
PACKET_COUNT = 0
MODE = 'F'
...
for onePort in ports:
    if "Arduino" in str(onePort):
        portVar = str(onePort)[0:4]
...
#Row layouts
first_row = [[sg.Text('Team ID: '+ TEAM_ID, key = 'id'), sg.Text('Packet Count: ' + str(PACKET_COUNT), key = 'packet_count'), sg.Text('Mission Time: ' + getTime(), key='time'), sg.Button('Connect', size = (8,1), button_color='#00b300'), sg.Button('Start'), sg.Button('Calibrate'), sg.Button('Close', button_color='#DA3A3A')]]
...
fourth_row= [sg.Text(' '*15), sg.Button('Save CSV', size = (10,1)), sg.Button('Simulation Enable', key = 'simEnable', size = (20,1)), sg.Button('Simulation Activate', key = 'simActivate', size = (20,1), visible = False)]
...
while True:
    ...
    # Figure out how to transmit the data
    if realSimulationMode == True:
         if (datetime.now() - start).seconds == seconds: # Every time a second passes 
            try:
                print(seconds, simulatedPressureData[seconds])
                try:
                    serialInst.write(simulatedPressureData[seconds].encode('utf-8'))
                except:
                    print("Can't send the data -- not connected to the Arduino!")
            except:
                print("No more pressure data to send")
                realSimulationMode == False
            seconds += 1    

    if simulationMode == True: # If simulation mode is active
        _VARS['window']['simActivate'].update(visible = True)
        if (datetime.now() - start).seconds == seconds: # Every time a second passes 
            if seconds == 1 or seconds % 10 == 0 and seconds < 121:       # If it's the first second of simulation or every 10 seconds, update all of the graphs
    ...
    if event == sg.WIN_CLOSED or event == 'Close': #If user closes window or clicks cancel
        break
    elif event == 'simActivate': # If user activates simulation mode
        simulationMode = True
        start = datetime.now()
        seconds = 0
    elif event == 'simEnable': # If user enables simulation mode
    ...
    elif event == 'Send': # If user tries to send a command
    ...
    # Eventually change this with simulation mode
    elif event == 'Calibrate': 
        realSimulationMode = not realSimulationMode
